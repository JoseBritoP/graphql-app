  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col rounded-md text-black gap-4 p-4 w-2/5 bg-slate-950 h-60 py-6'>
      ...
      <div className='flex items-center gap-x-4'>
        ...
        <input type="text" name='name' id='name' className='px-4 py-1 rounded-sm' placeholder='Write a title' onChange={handleChange} value={form.name}/>
      </div>
      <div className='flex flex-col gap-y-2'>
        <label htmlFor="description" className='text-white text-lg'>Description</label>
      <textarea name="description" id='description' placeholder='Write a description' className='resize-none px-4 py-1 rounded-sm' onChange={handleChange} value={form.description}/>
      </div>
